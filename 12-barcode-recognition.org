...
developed in
[[file:code-case-study-parsing-a-binary-data-format.html][Chapter 10, /Code case study: parsing a binary data format/]] to
...
later, in [[file:advanced-library-design-building-a-bloom-filter.html#bloomfilter.uarray][the section called “Unboxing, lifting, and bottom”]].
...
return to later, in
[[file:advanced-library-design-building-a-bloom-filter.html][Chapter 26, /Advanced library design: building a Bloom filter/]].
#+END_NOTE
...
[[file:code-case-study-parsing-a-binary-data-format.html][Chapter 10, /Code case study: parsing a binary data format/]].
...
[[file:code-case-study-parsing-a-binary-data-format.html][Chapter 10, /Code case study: parsing a binary data format/]]. The
...
[[file:barcode-recognition.html#barcode.fold][the section called “Folding over arrays”]].
...
in [[file:barcode-recognition.html#barcode.array][the section called “Introducing arrays”]]?
...
As we mentioned in [[file:barcode-recognition.html#barcode.array.mutable][the section called “Modifying array elements”]],
...
which two are particularly widely used. The ~lookup~ function has
a slightly tricky type signature, but don't worry; all will become
clear shortly, in [[file:monads.html][Chapter 14, /Monads/]].
...
Most often, the type parameter ~m~ in the result is ~Maybe~. In
...
The ~insertWith'~ function takes a further /combining function/ as
its argument. If no matching key was present in the map, the new
value is inserted verbatim. Otherwise, the combining function is
called on the new and old values, and its result is inserted into
the map.
...
avoid space leaks. While there exists a lazy variant (~insertWith~
without the trailing tick in the name), it's rarely what you
actually want.
...
[[file:data-structures.html][Chapter 13, /Data Structures/]]. For further inspiration, we
...
[[file:barcode-recognition.html#barcode.encode][the section called “EAN-13 encoding”]], we remember that the check
