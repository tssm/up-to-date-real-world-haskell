...
The ~=~~ operator uses typeclasses for both of its arguments, and
...
the base library defines many instances of this typeclass for us.
The ~Bool~ type is an instance of this typeclass, so we get back a
...
subject of exceptions in chapter [[file:error-handling.html][Chapter 19, /Error handling/]].
...
exercise [[file:efficient-file-processing-regular-expressions-and-file-name-matching.html#ch07.q.error][Q: 1]]). The ~error~ throws an exception. Pure Haskell code
