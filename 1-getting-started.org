...
in wide use. Hugs is an interpreter that is primarily used for teaching.
...
- ~ghc~ is an optimizing compiler that generates fast native
  code.
...
unmodified with older versions. However, we /recommend/ using the newest
version available for your platform. If you're using Windows or Mac OS
X, you can get started easily and quickly using a prebuilt installer. To
obtain a copy of GHC for these platforms, visit
[[http://www.haskell.org/ghc/download.html][the GHC download page]], and
...
~Prelude>~ prompt. Here, we're showing version 6.8.3 on a Linux box.
